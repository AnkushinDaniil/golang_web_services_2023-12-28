...
func TestSearchServer(t *testing.T) {
	...
	output := bufio.NewWriter(testFile)
	output.WriteByte('[')
	...
	for i := 0; i < len(cases); i++ {
		t.Run(fmt.Sprintf("Example №%v", i+1), func(t *testing.T) {
			...
			// fmt.Println(err)
			// assert.Equal(t, cases[i].Output, string(respBytes))
